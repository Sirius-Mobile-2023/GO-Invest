...
struct QuoteListResult: Decodable {
    ...
    func toQuotes() -> [Quote] {
        var quotes: [Quote] = []
        guard let idQuoteIndex = history.columns.firstIndex(of: QuoteListResult.Constants.idJsonName) else {
            return []
        }
        guard let nameQuoteIndex = history.columns.firstIndex(of: QuoteListResult.Constants.nameJsonName) else {
            return []
        }
